    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-6xl md:max-h-[90vh] max-h-[95vh] overflow-y-auto rounded-t-[2rem] md:rounded-[2rem] bg-[#111] border border-white/10 shadow-[0_50px_100px_rgba(0,0,0,0.8)]"
          onClick={e => e.stopPropagation()}
        >
          {/* 顶部栏 */}
          <div className="sticky top-0 z-20 flex items-center justify-between px-4 md:px-8 py-3 md:py-4 border-b border-white/5 bg-[#0a0a0a]">
            <div className="flex items-center gap-3">
              ...
              <span className="text-white/40 text-xs font-mono hidden md:inline">JIAYU // PORTFOLIO VIEWER</span>
            </div>
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="lg:w-2/3 relative bg-black">
              {work.category === 'film' && work.videoUrl ? (
                ...
                <div className="relative aspect-video">
                  ...
                  {images.length > 1 && (
                    <>
                      <button onClick={() => setImageIndex((imageIndex - 1 + images.length) % images.length)} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white border border-white/10">
                        <ChevronLeft size={20} />
                      </button>
                      <button onClick={() => setImageIndex((imageIndex + 1) % images.length)} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white border border-white/10">
            ...
            {/* 信息面板 */}
            <div className="lg:w-1/3 p-5 md:p-8 flex flex-col lg:border-l border-t lg:border-t-0 border-white/5 bg-[#0a0a0a]">
              <div className="flex-1">
                ...
                <h2 className="text-2xl md:text-4xl text-white mb-4" style={{ fontFamily: "'Instrument Serif', serif" }}>{work.title}</h2>
